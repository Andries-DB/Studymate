<template>
  <div class="w-full h-screen">

    <video autoplay muted loop plays-inline class="absolute -z-10 max-w-full">
      <source src="../../../public/assets/studying_background.mp4" type="video/mp4">
    </video>

    <nav class="flex justify-between items-center px-16 z-10">
      <div class="sidenav flex justify-center items-center text-3xl mt-5 text-indigo-500">
        Studym
        <span>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-8 h-8">
            <path d="M10.75 16.82A7.462 7.462 0 0115 15.5c.71 0 1.396.098 2.046.282A.75.75 0 0018 15.06v-11a.75.75 0 00-.546-.721A9.006 9.006 0 0015 3a8.963 8.963 0 00-4.25 1.065V16.82zM9.25 4.065A8.963 8.963 0 005 3c-.85 0-1.673.118-2.454.339A.75.75 0 002 4.06v11a.75.75 0 00.954.721A7.506 7.506 0 015 15.5c1.579 0 3.042.487 4.25 1.32V4.065z" />
          </svg>
        </span>
        te
      </div>
      <div class="flex gap-4 items-center justify-center text-indigo-500 mt-5">
        <a class="bg-indigo-500 py-2 px-4 text-gray-300 rounded-lg
      hover:bg-gray-300 hover:text-indigo-500" :href="route('register')">Registreren</a>
        <a class="bg-indigo-500 py-2 px-4 text-gray-300 rounded-lg
      hover:bg-gray-300 hover:text-indigo-500" :href="route('login')">Login</a>
      </div>
    </nav>
  </div>
</template>
